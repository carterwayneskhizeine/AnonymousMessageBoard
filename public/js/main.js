document.addEventListener('DOMContentLoaded', () => {
    ...
    // ========================================
    // GLOBAL DOM ELEMENT EXPOSURES
    // ========================================

    // Authentication DOM elements (for auth-handlers.js)
    ...
    // Core UI elements (for initial-setup.js and other modules)
    ...
    // File preview elements (for file-preview.js)
    window.fileStatus = fileStatus;
    window.filePreviewContainer = filePreviewContainer;
    window.filePreviewContent = filePreviewContent;
    ...
    // Markdown converter with YouTube extension
    let extensions = [];

    // Check if youtubeExtension is properly loaded and formatted
    if (window.youtubeExtension &&
        typeof window.youtubeExtension === 'object' &&
        window.youtubeExtension.type &&
    ...
    // ========================================
    // GLOBAL STATE EXPOSURES (GETTERS/SETTERS)
    // ========================================
        ...
        try {
            ...
            if (response.ok) {
                ...
                if (window.updateUIForUser) {
                    ...
                    // Fallback: directly update user state if function not available
                    window.currentUser = data.user;
                    if (window.guestView && window.userView) {
                        window.guestView.classList.add('hidden');
                        window.userView.classList.remove('hidden');
                        if (window.usernameDisplay) {
                            window.usernameDisplay.textContent = data.user.username;
                        }
                    }
                }
                return data.user;
    ...
    // The following functions are now defined in separate modules:
    // - updateFilePreview (file-preview.js)
    // - updateUIForUser (auth-ui-helper.js)
    // - renderMessage (main-rendering-function.js)
    // - fetchAndRenderMessages (api-rendering-logic.js)
    // - All pagination functions (pagination.js)
    // - All message handling functions (various message-*.js files)
    // - All comment handling functions (various comment-*.js files)
    // - All authentication event handlers (auth-handlers.js)
    // - All UI setup and event listeners (initial-setup.js)
    ...
    if (window.fetchAndRenderMessages) {
        ...
        console.error('fetchAndRenderMessages function not found - reloading page...');
        // Fallback: reload page if critical function not available
        setTimeout(() => {
    ...
    // Comment styles are initialized in comment-styles.js
});